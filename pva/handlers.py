...
from .abstract_interface import HelpInfoOutput, UserOutput, UsersOutput, NoteOutput, NotesOutput
...
def handler_command_guesser(user_command: list, *_) -> str:
    ...
    """
    candidates = user_command[1:]  # list of words inputted by user
    commands = []
...
def handler_happy_birthday(user_command: list, contact_dictionary: AddressBook, _=None) -> str:
    """Bot show birthday for contacts."""
...
def handler_show_all(_, contact_dictionary: AddressBook, __) -> list:
    ...
    output = UsersOutput()
    return output.create_output(contact_dictionary)
...
def handler_add_phone(user_command: list, contact_dictionary: AddressBook, path_file: str) -> str:
    ...
    User must write name of contact with is already in contact dictionary and one or more phone."""
...
def handler_phone(user_command: list, contact_dictionary: AddressBook, _=None) -> str:
    ...
    User must write name of contact with is already in contact dictionary."""

    if len(user_command) == 1:
        return 'Command \'phone\' shows phone(s) for entered contact. Example:\nphone <username>\n'
...
def handler_show_note(user_command: list, note_book: NoteBook, _=None) -> str:
    ...
    output = NoteOutput()
    return output.create_output(value, note_book)
